...
const MessageCollection = () => {
    const MESSAGES = gql`
      query GetShows { messageCollection ( order:sys_publishedAt_DESC)  {items{
    title
    image
    message {json}
  page {slug}
...
if (itm.page){ return (<Link href={"/"+itm?.page.slug} className="facility--main__card-content__cta"> View more <i className="golftio-long-right-arrow"></i>
  </Link>)}
  else{(<p> </p>); }
}
...
return(
  ...
  <div className="facility--main__card">
      <div className="facility--main__card-thumb">
        <Link href="#">
       <Image src={itm.image[0].secure_url} alt="" height={itm.image[0].height} width={itm.image[0].height} />
        </Link>
      </div>
 ...
 {itm?.title}  
        ...
        <p className="secondary-text">  <Richtext content={itm.message.json} /></p>
